...
중요한 것은 완성도, 개발, 독창성 보다 `문제 정의`라는 것을 느끼고 배울 수 있었다. 아무리 독창적인 솔루션이라도 시장의 반응이 없을 수 있고, 아무리 시장성이 좋은 솔루션이라도 고객에게 다가가는 방법을 찾지 못한다면 의미가 없을 수 있다. 개발또한 마찬가지다. 애자일하게, 보완해가며 쌓아가는게 중요하다. 오늘 들은 심사평중에 인상 깊었던 말은 `솔루션은 언제든 바뀔 수 있다` 는 점. 시장을 명확히 정의 하는 과정에서 고객이 좁혀지고 그 고객에 따라 같은 시장에서도 접근방식이 다를 수 있다. 

다만 이런 것들은 경험을 통해 배우기 마련인데 언제나 기술적 우위만을 쫓던 내가 시장성, 고객을 생각하려니 고민이 이만저만이 아니다. 이번 TEU에서 특히 그런것을 더 느낀 것 같다. 할 수 있는 일들도 많고 가능한 일들도 많은데 부딪힌 현실, 이해관계, 제약, 자금문제 등. 연구직만을 평생의 꿈으로 삼아왔던지라 그런 문제는 내가 생각할게 아니라고 여겼었고 그 잔재가 아직까지 남아있는 모양이다. 그러나... 여전히 기술적 우위는 점해야 한다고 생각한다. 그러려면 문제를 똑같이 정의하더라도 보는 시각이 달라야하고 남들이 보지 못하는 부분을 파고드는 수 밖엔 없다. 그게 아니고서야 자본을 이길 수 없으니.
다시 돌아와 내가 자신있는 점은, '남들이 보지 못하는 부분'을 보는 것인데 이를 어떻게 하면 더 갈고 닦을 수 있을지 고민이 많다. 어찌되었건 teachable machine은 mvp로는 언제나 옳은 툴이고 이번에 open pose라는 좋은 라이브러리와 그 원리를 알게 되어 즐거웠다. 그 내용은 내일 TIL에 써보도록 하겠다.
...
### NEXTRISE 2022에 다녀오며 (1) neurovis

18년도 창립한 NeuroVIS 라는 기업에서 다음과 같은 비즈니스 모델을 구축했다.

> 살아있는 동물에서 미세투석(Microdialysis)을 활용하여 뇌 내 신경전달물질(Neurotransmitters), 신경펩타이드(Neuropeptides), 및 사이토카인 등을 수집하고 고성능 질량분석기(LC-MS/MS)를 통한 광범위한 신경화학물질 분석을 기반으로 하여 다양한 중추신경계 평가 모델을 플랫폼으로 구축

비임상 실험동물에 적용하여 신경정신질환 동물모델에서 바이오마커를 탐색하고 영장류로 적용범위를 확대하여 바이오마커 검증 (biomarker validation) 을 통한 신경정신 질환 치료제 개발을 진행한다. 미세투석이라는 개념이 생소한데 살아있는 동물의 뇌 및 다양한 조직에 미세투석 Probe를 이식하여 'extracellular fluid'에 존재하는 신경전달물질 및 신경펩타이드 등을 실시간 샘플링 하는 기술이라고 한다. 간단하게 '작은 영역에서의 화학물질 농도'를 측정하는 방식이라고 할 수 있겠다. 00년도 초반부터 연구되거나 사용된 기술로 최신기술이라고는 하기 어렵겠다. 해당 기업의 특수한점은 실험을 종합적이고 통합적으로 대리해준다는 점에 있는듯 하다.

- [BRIC 회사소개](https://www.ibric.org/biomarket/new_protech/compview.php?iduserid=761803&BackLink=L2Jpb21hcmtldC9uZXdfcHJvdGVjaC9jb21wbGlzdC5waHA/b3J0eT3rgpg=)
- [공식페이지](http://www.neurovis.kr/kr/)

TEU 팀의 기존 아이디어가 바이오마커 탐색을 통한 예후분석 플랫폼 구축이어서 관심을 두었는데, 아직 기술적인 부분에서의 고민이 떠나지 않는다. 임상에서의 자기보고는 데이터화 하기 쉽지 않고 그렇다고 뇌과학적인 측면에서 접근하면 기술적인 안정성에서 문제가 생기거나 허들이 높아져버리니 그 사이의 접점을 어떻게 찾아야할지 고민이다. 또는 이런 바이오마커, 신경전달물질 변화가 증상에 즉각적이고 가시적인 영향을 미친다면 '감수하려'할 사람도 있겠지만 또 그것도 아니라는게 문제다. 관련 자료를 찾고 고민할수록 왜 심각한 질병에 관심과 투자와 연구가 몰리는지 알게 되었다. TEU MED 2기 오리엔테이션 연자로 오신 안과 희귀질환을 연구하시는 선생님이 종종 떠오르는 요즘이다.
...
다만 함수 설명에 자주 나오는 단어 '제네릭'은 익혀야겠다 싶어 알아보기로 한다.
제네릭 Generic 은 포괄적이고 일반적이라는 뜻으로 의약 시장에서의 제네릭 의약품의 필요성을 알아볼 때 자주 들었던 단어다. 

프로그래밍 언어에서 제네릭은 '타입을 마치 함수의 파라미터처럼 사용하는 것'을 의미한다. (*[타입스크립트 핸드북](https://joshua1988.github.io/ts/guide/generics.html)*, 제네릭을 검색하면 전부 타입스크립트만 나오는데 자료형에 엄격하기 때문일까? 자료형과 무슨 상관일까?)

제네릭은 C#, Java 등에서 *재사용성이 높은 컴포넌트를 만들 때* 자주 사용되며 한가지 타입보다 **여러가지 타입**에서 동작하는 컴포넌트를 생성하는데 사용된다.

타입스크립트에서는 함수의 인자로 들어갈 수 있는 자료형을 결정할 수 있는데 (e.g. python 3.10 에서 도입된 function annotation, or typing 문법) 함수 바로 뒤에 `<T>` 를 추가함으로써 함수에 넘기는 인자를 추정할 수 있도록 한다.
...
// 호출
const text = longText<string>("Hello Generic");
```

다음과 같이 타입을 추정할 때 사용되는 것 같다. 사실 찾으려고 했던건 MDN의 표현이었지만 지금 보고 나니 단순 번역 문제인것으로 보인다.


- *원래 찾으려고 했던 것*
<img width="600" alt="image" src="https://user-images.githubusercontent.com/60145951/174802300-ece0b4e6-2a6a-42bd-9a3a-32bdb42b1218.png">

- [출처: 타입스크립트 핸드북](https://joshua1988.github.io/ts/guide/generics.html)
